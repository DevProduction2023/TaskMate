...
import { addtodolistbyName } from '../hooks/AddList';
import { addTaskToTaskColumn } from '../hooks/addtask';
import { fetchTasksForPlaylist, fetchUserPlaylists } from '../hooks/fetchListData';
...
function ListAndTask() {
    ...
    return (
        <div className='px-4 my-5 h-full w-full'>
            <div className='w-full flex flex-col lg:flex-row justify-between lg:space-x-1 space-y-1'>
                <div className=' w-full lg:w-1/2 border-2 border-gray-400 rounded-md p-3 overflow-hidden'>
                    <div className='w-full flex flex-col lg:flex-row justify-between'>
                        <h1 className='px-2 font-semibold text-gray-600'>To-Do Lists</h1>
                        <div className='flex space-x-1'>
                            <input onChange={(e) => setListTitle(e.target.value)} value={listTitle} placeholder='Enter List Title' type="text" className='w-full border border-black/40 rounded-md px-3 py-1 shadow-black focus:outline-none focus:border-blue-500 focus:drop-shadow-xl text-gray-600' />
                        ...
                        <li className='hidden'></li>
                        {
                            toDoList === null ? '' :
                                toDoList.map((item, index) => (
                                    <div key={item?.id}>
                                        <li 
                                        onMouseEnter={() => handleListChange(item?._id)} 
                                        onMouseLeave={() => { setHoverListId(''), 
                                        clearTimeout(hoverTimeout.current); }} 
                                        onClick={() => setSelectTaskId(item?._id)} 
                                        className={`border cursor-pointer hover:bg-blue-500 hover:text-white rounded-md px-2 py-1 hover:ring-1 hover:ring-blue-400 ${selectTaskid === item?._id ? "bg-blue-500 text-white ring-1 ring-blue-500 font-semibold" : ''}`}>{item.title}</li>
                                    </div>
                ...
                <div className='w-full lg:w-1/2 grid border-2 border-gray-400 rounded-md p-3 overflow-hidden'>
                    <h1 className='w-full px-2 font-semibold text-gray-600'>Add New Task</h1>
                    <div className=' w-full flex my-2'>
                        <hr className='h-1 bg-gray-600 rounded-md mx-2 w-[15%]' />
                        <hr className='h-1 bg-gray-600 rounded-md w-[3%]' />
                    </div>
                    <ul className='w-full'>
                        <li className='w-full flex flex-col space-y-4 py-3'>
                            <input onChange={(e) => setTaskTitle(e.target.value)} value={taskTitle} type="text" name="taskTitle" id="" placeholder='Task Title' className='w-full border-b-2 border-black/40 rounded-md px-3 py-1 shadow-black focus:outline-none focus:border-blue-500 focus:drop-shadow-xl text-gray-600' />
                            <textarea name="" id="" onChange={(e) => setTaskDes(e.target.value)} value={taskDes} placeholder='Description' className='w-full border-b-2 border-black/40 rounded-md px-3 py-1 focus:outline-none focus:border-blue-500 focus:drop-shadow-xl text-gray-600'></textarea>
                        </li>
                        <li className='w-full flex flex-col lg:flex-row px-3 lg:mt-5 lg:space-x-2 space-y-1'>
                            <label className='py-1 font-semibold text-gray-700' htmlFor="date">Due Date</label>
                            <input value={taskDue_date} onChange={(e) => setDueDate(e.target.value)} className='w-full border-2 px-2 py-1 rounded-md border-black/40 focus:outline-none focus:border-blue-500 ' type="date" name="date" id="" placeholder='Task Title' />
                            <label className='py-1 font-semibold text-gray-700' htmlFor="task">Task Priority</label>
                            <select defaultValue={'low'} value={taskPriority} onChange={(e) => setPriority(e.target.value)} className=' w-full border-2 px-2 py-1 rounded-md border-black/40 focus:outline-none focus:border-blue-500 text-gray-600 font-semibold' name="Task Priority" id="">
