...
// import { updatedragTodoList } from '../hooks/firebase';
import { updatedragTodoList, addTaskToTaskColumn } from '../hooks/fetchListData';
...
function DragDrop({ taskColumbs, ListName, userUid, ListId, onChangeONDragTask, taskPriority, listTaskAddedStatus, onlistTaskAddedStatu, updateList }) {
    ...
    const [onDragStartTask, setOnDragStartTask] = useState(null)
                                    ...
                                    <Draggable key={item?._id} draggableId={item?._id} index={index}>
                                        {(provided, snapsho) => (
                                            <div
                                                ...
                                                className={`${taskId && taskId === item?._id ? 'w-20 h-20 flex' : "bg-white border rounded-md border-blue-200"}`}
                                            >{
                                                    taskId && taskId === item?._id ?
                                                        <div className='w-10 h-10 flex items-center justify-center' >
                                                            <img src='./drag.png' alt='drag img' className='w-full h-full object-contain'/>
                                                        </div >
                                                        :
                                                        <div className='px-4 py-2 flex flex-col justify-between w-full hover:bg-blue-200 hover:rounded-md cursor-grab ' >
                                                            <h1>{item?.title}</h1>
                                                            <h2>{item?.Task_des}</h2>
